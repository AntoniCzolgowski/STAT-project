...
>
> **Critical rules for all writeups** (see §14 for full template):
> 1. **Concise**. Each writeup ≈ 1 page rendered. The final paper is 5–8 pages total — writeups feed it, don't bury it.
> 2. **Always log human collaboration.** Every writeup must include a section recording the human's inputs, decisions, and questions that shaped that phase (and explicitly where the human did NOT intervene). This is what lets the human's teammates — who aren't in the Claude Code loop — write the paper's AI-evaluation section. Without these logs, the project's human-evaluation requirement can't be met.
...
**Length discipline**: each writeup targets ~1 page rendered (≈300–500 words of prose + small tables). The final paper is 5–8 pages. Writeups feed into the paper — they must be concise, not exhaustive.
...
## Key results
Tables / figures inline. No narration — just the numbers.

## --- HUMAN EVALUATION SECTIONS (keep tight) ---

### Decisions made in this phase
Bullet list. Each bullet: one line.

### Assumptions relied on
Bullet list. One line each.

### Alternatives considered but not chosen
Bullet list. Format: "[alternative] — [why not], one line".

### Risks / things a human should double-check
Bullet list.

### Code quality self-assessment
2–3 sentences max. What worked, what was uncertain.

## --- HUMAN COLLABORATION LOG ---

### Human inputs / decisions that shaped this phase
What the human explicitly told me to do (or not do) that affected the phase.
Format: "[what the human said or decided] → [how it changed the analysis]"
Bullet list.

### Human questions answered in-chat
Questions the human asked about this phase and the short answers I gave.
Format: "Q: ... / A: ..."
Bullet list. Max ~5 entries.

### Where the human did NOT intervene
Sections of this phase where I made decisions autonomously without human input.
This matters for the final AI-evaluation writeup.
Bullet list.
```

**Why the collaboration log matters**: the final paper must document the human evaluation of AI work (syllabus requirement). These logs are the raw material for the teammates writing the paper — they must be able to reconstruct "where the human drove vs. where the AI drove" just by reading the writeups. Without this log, that evaluation is impossible to write.

**What counts as "human input" to log:**
- Direction choices (e.g., "use Direction E", "exclude NBA-TV")
- Parameter choices (e.g., "use LMG, not PMVD")
- Corrections (e.g., "that team name is wrong, fix it")
- Rejections (e.g., "don't add that robustness check, keep scope tight")
- Clarifying questions the human asked that changed the approach

**What does NOT need logging**: trivial acknowledgments ("OK", "proceed"), one-word greenlights for the next phase if nothing changed.
